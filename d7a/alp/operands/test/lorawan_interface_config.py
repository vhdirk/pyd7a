...
class TestLoRaWANInterfaceConfiguration(unittest.TestCase):
  def test_byte_generation(self):
    ...
    self.assertEqual(bytes[42:50], bytearray([0] * 8))  # device EUI
    self.assertEqual(bytes[50:58], bytearray([0] * 8))  # app EUI
    self.assertEqual(bytes[58:74], bytearray([0] * 16))  # app key
  ...
  def test_parsing(self):
    ...
    self.assertEqual(config.device_eui, bytearray([0] * 8))
    self.assertEqual(config.app_eui, bytearray([0] * 8))
    self.assertEqual(config.app_key, bytearray([0] * 16))

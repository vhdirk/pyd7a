...
class TestLength(unittest.TestCase):
  def test_byte_generation(self):
    ...
    self.assertEqual(bytes[0], 1 << 6) # control byte
    ...
    self.assertEqual(bytes[34:38], bytearray([3] * 4))  # dev addr
    self.assertEqual(bytes[38:], bytearray([4] * 4))  # netw id
  ...
  def test_parsing(self):
    ...
    bytes.extend([2] * 4) # dev addr
    bytes.extend([3] * 4)  # netw id
    ...
    self.assertEqual(config.dev_addr, bytearray([2] * 4))
    self.assertEqual(config.netw_id, bytearray([3] * 4))
